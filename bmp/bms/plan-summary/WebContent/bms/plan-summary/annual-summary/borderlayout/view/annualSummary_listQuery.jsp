...
<script type="text/javascript">
	...
	function doEdit(action) {
		...
		window.location.href = action+"?annualSummary.annualSummaryId=" + items[0].value + "&_ts="+ new Date().getTime();
	}
	...
	function doDetail(id) {
		if( id!=null && id!="" ){
				$ENV.dialog.open({
					url : "${ctx}/bmp/annualSummary/annualSummary_detail.action?annualSummary.annualSummaryId=" + id + "&_ts=" + new Date().getTime(),
					width : 0.8,
					height : 0.9,
					title : '详情'
				});
	...
	<div class="body_content">
		...
		<div class="panel">
			<div id="cpPanel" style="width: 100%;">
				<!-- 切换按钮 -->
				<div class="cpBtnBar">
					<div id="cp001Btn" class="cpBtn_ov" href="###" onmouseover="javascript:showCp(2,'cp001','年度工作总结简介');">关 于</div>
					<div id="cp002Btn" class="cpBtn" href="###" onmouseover="javascript:showCp(2,'cp002','年度工作总结查询');">查 询</div>
				</div>
				<!-- 切换面板 -->
						...
						<tr>
							...
							<td width="*" valign="top">
								<div id="cp001"
									...
									<!-- 模块简介 -->
									<div class="cpMsgTitle">关于保密委员会（保密工作领导小组）年度工作总结</div>
								...
								<div id="cp002"
									...
									<form method="post" id="queryform" action="${ctx }/bmp/annualSummary/annualSummary_listQuery.action">
									    <input name="checkLower" value="${checkLower}" type="hidden"/>
									    <input name="fromQuery" value="1" type="hidden"/>
									    <input name="district.districtCode" value="${district.districtCode}" type="hidden"/>
										<table class="st" width="100%">
											<tr>
												<td class="tbLable fr">名称：</td>

												<td class="tbValue fl"><input name="annualSummary.annualSummaryTitle" value="${annualSummary.annualSummaryTitle }" type="text"/></td>
												<td class="tbLable fr">年度：</td>
												<td class="tbValue fl">
													<input type="text" name="annualSummary.annualSummaryYear" value="${annualSummary.annualSummaryYear }" readonly="readonly" class="Wdate" onFocus="WdatePicker({dateFmt:'yyyy'})" />
												</td>
											</tr>
											<tr>
			...
			<div class="panel_header">
				<div class="panel_title panel_titleListIco">
					...
					<c:if test="${checkLower ne '1'}">
						${district.districtName}<dis:levelText district="${district}"></dis:levelText>级机关单位 - 年度工作总结列表
					</c:if>
					<c:if test="${checkLower eq '1'}">
						${district.districtName} - 年度工作总结列表
					</c:if>
			...
			<div class="panel_content panel_content_table">
				...
				<s:property value="request.dataList.size"/>
					<ec:table items="dataList" var="dataRow" tableId="dataList" border="0"
						...
						<ec:row>
							<ec:column property="annualSummaryId" alias="annualSummaryId_checkbox" cell="checkbox" headerCell="checkbox" width="5%"/>
							<ec:column property="annualSummaryTitle" title="年度工作总结标题" width="20%" cell="text" alias="size=25"/>
							<ec:column property="annualSummaryYear" title="年度" width="10%" />
							...
							<ec:column property="null" title="详 情" width="10%">
								<a href="###" onclick="doDetail('${dataRow.annualSummaryId}');"><img src="${ctx}/platform/template/borderlayout/skin/blue/img/btn/display.gif" border="0" title="显示详情"/></a>
							</ec:column>
						</ec:row>
					</ec:table>
				</s:if>
