			...
			$ENV.loader.loadJavaScript("${ctx}/resources/js/miftree/1.2/mif.tree.js",function(){
				$ENV.onDomReady(function(){
					...
					tree.addEvent('select',function(node){
						...
						contentLoad("${ctx}/bmp/departmentWebsite/departmentWebsite_listQuery.action?district.districtCode="+node.data.id+"&fromQuery=1&checkLower="+chkIncludeChild+"&_ts"+$time());
					});
					// 最后的10是上下边距和
		...
		<script type="text/javascript">
			function doChangeValue(value){
				if(value == 1){
					...
					contentLoad("${ctx}/bmp/departmentWebsite/departmentWebsite_listQuery.action?district.districtCode="+$('district_code').value+"&fromQuery=1&checkLower=0&_ts"+$time());
				} else {
					$('chkIncludeChild').value = 1;
					contentLoad("${ctx}/bmp/departmentWebsite/departmentWebsite_listQuery.action?district.districtCode="+$('district_code').value+"&fromQuery=1&checkLower=1&_ts"+$time());
				}
			}
		</script>
	...
	<body>
		...
		<div id="body_content" class="body_content">
			...
			<div class="tree_right_content"><!-- 右边表的内容  src=初始化内容 -->
				<iframe name="contentIframe" id="contentIframe" scrolling="auto" src="${ctx}/bmp/departmentWebsite/departmentWebsite_listQuery.action?fromQuery=1&checkLower=1" height="100%" width="100%" frameborder="0"></iframe>
			</div>
		</div>
	</body>
</html>