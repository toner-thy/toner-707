		...
		<script type="text/javascript">
					...
					if(!item.content.getContentWindow().needReload2){
						if (window.confirm("您确定放弃正在新增的内容吗？")) {
							if(item.content.getContentWindow().needReload){
								document.getElementById('secrecyTechnologyPrevention_list_form').submit();
							}
						} else {
							return false;
						}
			...
			function doPrint(action){
		    	var item = EcTable.getCheckedItems();
		     	if(item!=null && item!=""){
			    	window.showModalDialog( action+'?secrecyTechnologyPrevention.id='+item[0].value+'&t_time='+new Date().getTime(),window,"dialogWidth=630px;dialogHeight=400px;status=no;directories=no;menubar=no;resizable=yes;crollbars=auto;help=no");
		     	}else{
       		...
       		//导出
       	  	function doExport(action){
       	     	$('secrecyTechnologyPrevention_list_form').submit();
		     	$('secrecyTechnologyPrevention_list_form').action="<s:url action="secrecyTechnologyPrevention_list.action" includeParams="true"/>";
       	  	}
		</script>
		   ...
		   <!-- 复合面板开始 -->
			<cp:start defaultTitle="设备分类简介" ctx="${ctx}" icoPath="/bmp/checkevent/borderlayout/skin/blue/img/list_.gif">
				...
				<cp:search show="true" divId="cp002">
					<form action="secrecyTechnologyPrevention_list.action" id="secrecyTechnologyPrevention_list_form" name='queryform'>
			...
			<div class="panel tMargin">
				...
				<s:if test="#request.list.size>0">
					<ec:table items="list" var="secrecyTechnologyPrevention" tableId="list" border="0"
						action="${ctx}/bmp/secrecyTechnologyPrevention/secrecyTechnologyPrevention_list.action"
