		...
		<script type="text/javascript">
			...
			$ENV.loader.loadJavaScript("${ctx}/resources/js/formcheck/1.4/formcheck.js",function(){
				$ENV.onDomReady(function(){
					...
					formcheck = new FormCheck('form_secrecyMaintain_update',{
								display:{
			...
			//添加表格
			function doAddTable(){
				window.showModalDialog("secrecyMaintain_selectTable.action"
					,{window:window,
					text:'tableText',
					hidden:'tableIds'
			...
			function doCheckTable(){
				...
				window.location.href="${pageContext.request.contextPath}/checkEventTable_list.action?checkEventTable.checkEventTableId="+items[0].value+"&secrecyMaintain.id=${secrecyMaintain.id}";
				//alert("${pageContext.request.contextPath}/checkEventTable_list.action?checkEventTable.checkEventTableId="+items[0].value+"&secrecyMaintain.id=${secrecyMaintain.id}");
			}
			//保存
			function doSave(){
	 ...
	 <!-- 公共头部 -->
		...
		<!-- 内容panel开始 -->
	 		<div class="panel">
				...
				<form id="form_secrecyMaintain_update" class="form"  action="<s:url namespace='/bmp/secrecyMaintain' action='secrecyMaintain_update' includeParams='true'/>" method="post" enctype="multipart/form-data">
					<table class="content_table">
						...
						<input type="hidden" id="secrecyMaintain.id" name='secrecyMaintain.id' value="${secrecyMaintain.id}" class="btn_23" style="display: none;"/>
