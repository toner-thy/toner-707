...
<body>
		<div id="body_content">
			...
			<div class="tree_right_content" style="border: 1px solid #B5CBD9;">
				<iframe name="checkEventIframe" id="checkEventIframe" scrolling="auto" src="${ctx}/bmp/secrecyTechnologyPrevention/secrecyTechnologyPrevention_allList.action" height="100%" width="100%" frameborder="0"></iframe>
			</div>
			<!-- 右侧内容结束 -->
		</div>
	</body>
...
<script type="text/javascript">
	...
	tree.addEvent('select',function(node){
		...
		contentLoad("${ctx}/bmp/secrecyTechnologyPrevention/secrecyTechnologyPrevention_allList.action?district.districtCode="+node.data.id+"&district.districtName="+node.name+"&showType="+chkIncludeChild+"&_ts"+$time());
	});
...
<script type="text/javascript">
			function doChangeValue(value){
				if(value == 0){
					...
					contentLoad("${ctx}/bmp/secrecyTechnologyPrevention/secrecyTechnologyPrevention_allList.action?district.districtCode="+$('district_code').value+"&district.districtName="+$('district_name').value+"&showType=1&_ts"+$time());
				} else {
					//包含
					$('chkIncludeChild').value = 0;
					contentLoad("${ctx}/bmp/secrecyTechnologyPrevention/secrecyTechnologyPrevention_allList.action?district.districtCode="+$('district_code').value+"&district.districtName="+$('district_name').value+"&showType=0&_ts"+$time());
				}
			}
</script>