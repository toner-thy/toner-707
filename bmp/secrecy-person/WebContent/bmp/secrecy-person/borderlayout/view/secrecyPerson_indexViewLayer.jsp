...
<html>
	<head>
		...
		<script type="text/javascript">
 			function doSecView1(action, type) {
 				TabUtil.openAsTab({
 			...
 			function doSecPersonView1(id,secrecyLevel){
 				TabUtil.openAsTab({
 					url : '${ctx}/bmp/secrecyperson/secrecyPerson_layerSecPersonData.action?id='+id+'&secrecyLevel='+secrecyLevel+'&t_date='+ new Date().getTime(),
 					title : '涉密人员-列表'
 				});
 			}
								...
								<table style="width: 100%;" cellpadding="0" cellspacing="0">
										...
										<c:forEach var="option" items="${optionList }">
											<tr>
												...
												<td class="td_content_right">
													<c:choose>
														<c:when test="${not empty secrecyPersonCountMap }">
															<c:forEach var="aKeyValue" items="${secrecyPersonCountMap }" >
																<c:if test="${aKeyValue.key eq option.optionValue }">
																	<c:choose>
																		<c:when test="${aKeyValue.value ne 0 }">
																			<a href="###" onclick="doSecPersonView1('${organ.organId}','${aKeyValue.key }')">
																				${aKeyValue.value}
																				<c:set var="secrecyPersonNum" value="${secrecyPersonNum +  aKeyValue.value}" />
																			</a>
																		</c:when>
																		<c:otherwise>
																			0
									...
									<tr align="right">
										...
										<td class="td_total_right">
											<c:choose>
												<c:when test="${secrecyPersonNum eq 0}">
													0
												</c:when>
												<c:otherwise>
													<a href="###" onclick="doSecView1('${ctx}/bmp/secrecyperson/secrecyPerson_layerList.action?organ.organId=${organ.organId }', 'secrecyPerson')">
														${secrecyPersonNum}
													</a>
												</c:otherwise>
											</c:choose>
