...
<script type="text/javascript">
function doExportCurrentPartNation(){
		var districtCode;
		...
		window.location.href = "${ctx}/bmp/secrecyperson/secrecyPerson_exportCurrentNation.action?districtCode=" + districtCode + "&countScope=${countScope}" + "&_ts=" + $time();
}
</script>
		...
		<s:if test="#request.nationStatisticsList.size>0">
				<div class="panel_header">
					...
					<div class="panel_btn_bar pop_button_bar">
						<c:if test="${expBtnFlag eq 'currentPart' }" >
							<a class="pop_button" href="###" onclick="doExportCurrentPartNation()"><span>导出</span></a>
						</c:if>
					</div>
				</div>
				<div class="eXtremeTable" style="overflow: auto;">
					<table border="0"  cellspacing="0"  cellpadding="0"  class="tableRegion"  width="30%">
						<thead>
							<tr>
								<td align="center" style="font-weight:bolder;color:black;" rowspan="2">民族</td>
						...
						<c:forEach var="nsl" items="${nationStatisticsList }" varStatus="st">
							<c:forEach var="nationRow" items="${nsl }">
								<tr class="odd">
									<td align="center" nowrap="nowrap">${ nationRow.key}</td>
									<td align="center" nowrap="nowrap">
										<c:set var="flag" value="1" />
										<c:forEach var="nationMap" items="${nationRow.value }" >
											  <c:if test="${nationMap.key == 1 }" >
												${nationMap.value}
										...
										<c:forEach var="nationMap" items="${nationRow.value }" >
											 <c:if test="${nationMap.key == 1 }" >
												 <c:set var="v" value="${nationMap.value}"></c:set>
											 </c:if>
											 <c:if test="${nationMap.key == 4 }" >
												<c:set var="total" value="${nationMap.value}"></c:set>
											 </c:if>
										</c:forEach>
