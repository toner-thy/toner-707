...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function doAdd(){
			   TabUtil.openAsTab({
					url : "${contextPath}/bmp/secrecyperson/secrecyPerson_add.action?&departmentId=${secrecyPerson.department.departmentId}&nestedflag=1",
					title : '机关涉密人员-新增',
			...
			function doEditSecPerson() {
				...
				TabUtil.openAsTab({
					url : "${contextPath}/bmp/secrecyperson/secrecyPerson_edit.action?&secrecyPerson.secrecyPersonId="+items[0].value+"&departmentId=${secrecyPerson.department.departmentId}&&nestedflag=1",
					title : '机关涉密人员-编辑',
					onClose : function(tab, item) {
						if(!item.content.getContentWindow().needReload2){
			...
			function doDeleteSecPerson(){
				...
				if (window.confirm('确定要删除吗？ ' )) {
					...
					new Request.JSON({
					    url: "${contextPath}/bmp/secrecyperson/nested/secrecyPersonList_delete.action",
		...
		<div class="body_content" >
			...
			<cp:start defaultTitle="涉密人员简介" ctx="${ctx}" icoPath="/bmp/secrecy-person/borderlayout/skin/blue/img/list_cpIco.gif">
				...
				<cp:search show="false" divId="cp002">
					<s:form action="secrecyPerson_query_list.action" method="post" id="secrecyPersonForm" theme="simple">
				...
				<div class="panel_content panel_content_table">
					<s:if test="#request.secrecyPersonList.size>0">
						<ec:table items="secrecyPersonList" var="secrecyPerson" tableId="secrecyPersonList" border="0"
							action="${ctx}/bmp/secrecyperson/secrecyPerson_query_list.action"
