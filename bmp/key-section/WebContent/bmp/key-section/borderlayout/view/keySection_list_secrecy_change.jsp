...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function doDetailChange(id){
				$ENV.dialog.open({
					url : "${ctx}/bmp/keySection/keySection_changeDetail.action?keySectionChange.sectionChangeId=" + id +"&_ts="+new Date().getTime(),
					width : 0.8,
					height : 0.6,
					title : '要害部门密级变更详情'
				});
	...
	<body>
		...
		<div id="body_content" class="body_content">
	        <cp:start defaultTitle="保密要害部门密级变更历史记录简介" ctx="${ctx}" icoPath="/bmp/key-section/borderlayout/skin/blue/img/list_cpIco.gif">
				<div id="cp001Btn" class="cpBtn_ov" href="###" onmouseover="javascript:showCp(2,'cp001','保密要害部门密级变更历史记录简介');">关 于</div>
				<div id="cp002Btn" class="cpBtn" href="###" onmouseover="javascript:showCp(2,'cp002','保密要害部门密级变更历史记录查询');">查 询</div>
			</cp:start>
			...
			<cp:search show="false" divId="cp002">
				<form id="keySection_query_form_change" name="keySection_query_form_change" action="keySection_list_change.action" method="post">
					...
					<table class="st" width="100%">
						<tr>
							<td class="tbLable fr">保密要害部门：</td>
							<td class="tbValue fl">
								<input type="text" name="keySectionChange.keySectionId.department.departmentName" id="keySectionChange.keySectionId.department.departmentName" >
							</td>
							<td class="tbLable fr">原密级：</td>
							<td class="tbValue fl">
								<dictionary:select id="keySectionChange.beforeLevel" name="keySectionChange.beforeLevel" tableCode="bmp" fieldCode="secrecy_level_section"
								 title="true" titleText="全部" style="width:140px;"></dictionary:select>
							</td>
						</tr>
						<tr>
							...
							<td class="tbValue fl" >
								<input type="text" name="keySectionChange.changeDate" id="keySectionChange.changeDate" class="Wdate validate['length[20]'] w130"
						...
						<tr>
							<td colspan="4" class="fc" style="border:0px;">
								<div class="stBtnBar">
									<a class="pop_button" href="javascript:document.getElementById('keySection_query_form_change').submit();"><span>查 询</span></a>
									<a class="pop_button" href="javascript:document.getElementById('keySection_query_form_change').reset();"><span>重 置</span></a>
								</div>
							</td>
						</tr>
					</table>
					...
					<div class="panel_title panel_titleListIco">
						...
						<c:if test="${ywFlag eq '1'}">
							...
							<c:if test="${isChildren ne '1'}">
								${district.districtName}<dis:levelText district="${district}"></dis:levelText>级机关单位 - 保密要害部门密级变更列表
							</c:if>
							<c:if test="${isChildren eq '1'}">
								${district.districtName} - 保密要害部门密级变更列表
							</c:if>
						</c:if>
					</div>
				</div>
				...
				<div class="panel_content panel_content_table">
					<s:if test="#request.keySection_changelist.size>0">
						<ec:table items="keySection_changelist" var="keySectionChange" tableId="keySection_changelist" border="0"
							action="${ctx}/bmp/keySection/keySection_list_change.action"
							imagePath="${ctx}/platform/template/borderlayout/skin/blue/img/ec/*.gif"
							width="100%" filterable="false" autoIncludeParameters="true" sortable="false">
							<!-- retrieveRowsCallback="limit" sortRowsCallback="limit" -->
							<ec:row>
								<%-- <ec:column property="sectionChangeId" alias="keySectionChange_checkbox" cell="checkbox" headerCell="checkbox" width="5%"/> --%>
								...
								<ec:column property="null" title="详 情" width="10%">
									<a href="###" onclick="doDetailChange('${keySectionChange.sectionChangeId}');"><img src="${ctx}/platform/template/borderlayout/skin/blue/img/btn/display.gif" border="0" title="显示详情"/></a>
								</ec:column>
							</ec:row>
						</ec:table>
					</s:if>
