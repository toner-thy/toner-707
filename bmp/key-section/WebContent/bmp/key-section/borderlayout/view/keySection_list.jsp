...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function query() {
				var frm = document.getElementById('keySection_query_form');
				var flag = $("ywFlag").value;
				if(flag=="1") {//查询
					frm.action = "${ctx}/bmp/keySection/keySection_query_list.action";
				}else {
					frm.action = "${ctx}/bmp/keySection/keySection_list.action";
				}
	...
	<body>
		...
		<div class="body_content" >
			<div id="main_tab" class="panel tab_panel" style="height: 575px;width: 100%; overflow-y:auto;overflow-x:hidden;">
				<div id="main_tab_header" class="panel_header" style="width: 100%;">
					<div class="tab_bar">
						<div class="panel_title">保密要害部门列表</div>
						<div class="panel_title">保密要害部门密级变更历史列表</div>
						<div class="panel_title">保密要害部门解除部门信息列表</div>
					</div>
				...
				<div class="tab_panel_content" >
					...
					<div id="body_content" class="body_content" >
						...
						<cp:search show="false" divId="cp002">
							<form action="<s:url action="keySection_list" namespace="/bmp/keySection"/>" method="post" id="keySection_query_form" >
								...
								<table class="st" width="100%">
									<tr>
										...
										<td class="tbValue fl">
											<dictionary:select tableCode="bmp" fieldCode="secrecy_level_section" id="keySectionQo.secrecyLevel"
...
<!-- 					</div> -->
						...
						<div class="panel tMargin">
							<div class="panel_header">
								...
								<c:if test="${ywFlag eq '1'}">
									<!--判断是否查看下级1查看，0不查看-->
									<c:if test="${isChildren eq '0'}">
										${district.districtName}<dis:levelText district="${district}"></dis:levelText>级机关单位 - 保密要害部门列表
									</c:if>
									<c:if test="${isChildren eq '1'}">
										${district.districtName} - 保密要害部门列表
									</c:if>
								</c:if>
								</div>
							</div>
					...
					<!-- 保密要害部门密级变更历史列表 -->
					<div class="panel_content" url="${ctx}/bmp/keySection/keySection_list_change.action?districtCode=${district.districtCode}&ywFlag=${ywFlag}&isChildren=${isChildren}">
					</div>

					<!-- 保密要害部门解除部门信息列表 -->
					<div class="panel_content" url="${ctx}/bmp/keySection/keySection_list_clear.action?districtCode=${district.districtCode}&ywFlag=${ywFlag}&isChildren=${isChildren}">
					</div>
