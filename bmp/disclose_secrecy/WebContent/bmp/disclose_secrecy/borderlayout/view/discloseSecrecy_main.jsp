	...
	<head>
		...
		<script type="text/javascript">
					...
					if(!item.content.getContentWindow().needReload2){
						if (window.confirm("您确定放弃正在新增的内容吗？")) {
							if(item.content.getContentWindow().needReload){
								window.location.href ="${contextPath}/bmp/discloseSecrecy/discloseSecrecy_listMain.action";
							}
						} else {
							return false;
						}
			...
			function doSecrecyChange(action){
				...
				$ENV.dialog.open({
					url : action + "?discloseSecrecy.disclosesecrecycaseId=" + items[0].value + "&_ts="+new Date().getTime(),
					width : 0.5,
					height : 0.5,
					title : '泄密案件密级变更'
				});
		...
		<div class="body_content" id="body_content" >
			<div id="main_tab" class="panel tab_panel" style="height: 650px;width: 100%;">
				...
				<div class="tab_panel_content">
					<div class="panel_content" url="${ctx}/bmp/discloseSecrecy/discloseSecrecy_list.action">
					</div>
					<div class="panel_content" url="${ctx}/bmp/discloseSecrecy/discloseSecrecy_list_change.action">
					</div>
					<div class="panel_content" url="${ctx}/bmp/discloseSecrecy/discloseSecrecy_list_clear.action">
					</div>
