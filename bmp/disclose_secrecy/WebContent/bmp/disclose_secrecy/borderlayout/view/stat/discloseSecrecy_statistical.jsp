...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function doDownLoad(districtCode,zhiquyu,className){
				window.location.href="${pageContext.request.contextPath}/bmp/discloseSecrecy/zhtj_export.action?className="+className+"&&zhiquyu="+zhiquyu+"&&district.districtCode="+districtCode;
			}
		</script>
	...
	<body>
		...
		<div id="body_content" class="body_content">
			<div>
			<bmp:navigationBar district="${district}" topDistrict="${topDistrict}" url="${pageContext.request.contextPath}/bmp/discloseSecrecy/zhtj_query_Detail.action?className=${className}" />
			</div>
			<div class="panel">
				<div class="panel_header">
					<div class="panel_title panel_titleListIco">
						【${district.districtName }】<%=ConstantService.getConstant("District")%>
						${className eq 'com.cdthgk.disclosesecrecy.vo.DiscloseSecrecy'?'泄密案件统计':'严重违规案件统计'}
					</div>
					<!-- 右侧按钮 -->
					<div class="panel_btn_bar pop_button_bar" align="right">
						<a class="pop_button" href="###" onclick="doDownLoad('${districtCode}','false','${className eq 'com.cdthgk.disclosesecrecy.vo.DiscloseSecrecy'?'1':'2'}');"><span>导出</span></a>
						...
						<table border="0"  cellspacing="0"  cellpadding="0"  class="tableRegion"  width="100%" >
							...
							<tbody class="tableBody">
								<tr height="36px;" class="odd" onmouseover="this.className='highlight'" onmouseout="this.className='odd'">
									...
									<td><%=ConstantService.getConstant("SecrecyLevel_1")%></td>
									<td><%=ConstantService.getConstant("SecrecyLevel_2")%></td>
									<td><%=ConstantService.getConstant("SecrecyLevel_3")%></td>
									<td>合计</td>
									<td><%=ConstantService.getConstant("SecrecyLevel_1")%></td>
									<td><%=ConstantService.getConstant("SecrecyLevel_2")%></td>
									<td><%=ConstantService.getConstant("SecrecyLevel_3")%></td>
								...
								<c:forEach items="${keySectionStatDtoList}" var="o" varStatus="x">
									<tr height="36px;" class="${x.count % 2 == 0 ? 'odd' : 'even'}" onmouseover="this.className='highlight'" onmouseout="this.className='${x.count % 2 == 0 ? 'odd' : 'even' }'">
										<td><a href="${pageContext.request.contextPath}/bmp/discloseSecrecy/zhtj_queryByDistrict.action?className=${className}&&district.districtCode=${o.districtCode}">${o.name}</a></td>
										<td>${o.num1}</td>
										<td>${o.num2}</td>
										<td>${o.num3}</td>
										<td>${o.selfTotal}</td>
			...
			<br>
			<c:if test="${not empty distictList}">
				<!-- 列表 -->
				<div class="panel" style="margin-top: -15px;">
					<div class="panel_header">
						<div class="panel_title panel_titleListIco">
							<h2>
								【${district.districtName }】<%=ConstantService.getConstant("District_Area")%>
								${className eq 'com.cdthgk.disclosesecrecy.vo.DiscloseSecrecy'?'泄密案件统计':'严重违规案件统计'}
							</h2>
						</div>
						<!-- 右侧按钮 -->
						<div class="panel_btn_bar pop_button_bar" align="right">
							<a class="pop_button" href="###" onclick="doDownLoad('${districtCode}','true','${className eq 'com.cdthgk.disclosesecrecy.vo.DiscloseSecrecy'?'1':'2'}');">
							<span>导出</span>
							</a>
						...
						<div class="eXtremeTable" >
							<table border="0" cellspacing="0" cellpadding="0" class="tableRegion" width="100%" >
								...
								<tbody class="tableBody">
									...
									<c:forEach items="${distictList}" var="o" varStatus="x">
										<tr height="36px;" class="${x.count % 2 == 0 ? 'odd' : 'even'}" onmouseover="this.className='highlight'"  onmouseout="this.className='${x.count % 2 == 0 ? 'odd' : 'even' }'">
											<td>
												<c:if test="${o.districtCode == null}">${o.name}</c:if>
												<c:if test="${o.districtCode != null}"><a href="${pageContext.request.contextPath}/bmp/discloseSecrecy/zhtj_query_Detail.action?className=${className}&&district.districtCode=${o.districtCode}">${o.name}</a></c:if>
											</td>
											<td>${o.num1}</td>
											<td>${o.num2}</td>
											<td>${o.num3}</td>
