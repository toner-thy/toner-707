...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function doView(id,secrecyLevel,name){
				var className='${className}';
				var url;
				var title;
				if(className=='com.cdthgk.disclosesecrecy.vo.DiscloseSecrecy')
					{
					title='泄密案件统计详情';
					url='${ctx}/bmp/discloseSecrecy/discloseSecrecy_selectListing2.action?discloseSecrecy.createOrgan.organName='+name+'&discloseSecrecy.createOrgan.organId='+id+'&includeChild=0&discloseSecrecy.secrecyLevel='+secrecyLevel+'&districtCode='+${district.districtCode}+'&t_date=' + new Date().getTime();
					}
				else{
					title='严重违规案件统计详情';
					url='${ctx}/bmp/caseCriticalviolation/caseCriticalviolation_selectListing2.action?caseCriticalviolation.createOrgan.organName='+name+'&caseCriticalviolation.createOrgan.organId='+id+'&includeChild=0&caseCriticalviolation.secrecyLevel='+secrecyLevel+'&districtCode='+${district.districtCode}+'&t_date=' + new Date().getTime();
				}
		...
		<div class="button_bar">
			<div class="left">
				...
				<a class="pop_button" href="${pageContext.request.contextPath}/bmp/keysectionstat/keySectionStat_organColumnChartByDirectLy.action?district.districtCode=${district.districtCode}"><span>柱形图</span></a>
			     --%>
			     <a class="pop_button" href="${pageContext.request.contextPath}/bmp/discloseSecrecy/zhtj_query_Detail.action?className=${className}&district.districtCode=${district.districtCode}"><span>返回</span></a>
			</div>
			<div class="right">
						...
						<table class="panel_content_search_form">
				            <tr>
								...
								<td style="text-align:left;">
									...
									<input type="hidden" id="className" name="className" class="input1" value="${className}"/>
								</td>
								<td>
						...
						<table cellpadding="0" cellspacing="0" border="0" width="80%"  border="0"  cellspacing="0"  cellpadding="0"  class="tableRegion" >
							<thead>
								<tr>
									...
									<td class="tableHeader"><%=ConstantService.getConstant("SecrecyLevel_1")%></td>
									<td class="tableHeader"><%=ConstantService.getConstant("SecrecyLevel_2")%></td>
									<td class="tableHeader"><%=ConstantService.getConstant("SecrecyLevel_3")%></td>
							...
							<tbody class="tableBody">
								<c:forEach items="${secrecyCountryItemStatDtoList}" var="o"  varStatus="x">
									<tr height="36px;" class="${x.count % 2 == 0 ? 'odd' : 'even' }" onmouseover="this.className='highlight'"  onmouseout="this.className='${x.count % 2 == 0 ? 'odd' : 'even' }'">
										<td>${o.name}</td>
										<td>
											<c:if test="${o.num1==0}">${o.num1}</c:if>
											<c:if test="${o.num1>0}">
												<a href="###" onclick="doView('${o.id}','3','${o.name}')">${o.num1}</a>
											</c:if>
										</td>
										<td>
											<c:if test="${o.num2==0}">${o.num2}</c:if>
											<c:if test="${o.num2>0}">
												<a href="###" onclick="doView('${o.id}','2','${o.name}')">${o.num2}</a>
											</c:if>
										</td>
										<td>
											<c:if test="${o.num3==0}">${o.num3}</c:if>
											<c:if test="${o.num3>0}">
												<a href="###" onclick="doView('${o.id}','1','${o.name}')">${o.num3}</a>
											</c:if>
										</td>
										<td>${o.selfTotal}</td>
									</tr>
