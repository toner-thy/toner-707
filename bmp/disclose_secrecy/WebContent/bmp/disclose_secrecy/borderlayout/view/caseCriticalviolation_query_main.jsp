	...
	<body>
		...
		<div id="body_content" class="body_content">
				...
				<div id="district_tree_header" class="panel_header">
					...
					<div class="panel_title" style="float: right;">
						...
						<input id="chkIncludeChild" name="chkIncludeChild" type="checkbox" checked="checked"
						 value="1" style="margin: 0 5px;" onclick="doChangeValue(this.value)">
			...
			<div class="tree_right_content"><!-- 右边表的内容  src=初始化内容 -->
				<iframe name="contentIframe" id="contentIframe" scrolling="auto" src="${ctx}/bmp/caseCriticalviolation/caseCriticalviolation_selectListing.action" height="100%" width="100%" frameborder="0"></iframe>
			</div>
		</div>
	</body>
</html>
<script type="text/javascript">
	...
	tree.addEvent('select',function(node){
		...
		contentLoad("${ctx}/bmp/caseCriticalviolation/caseCriticalviolation_selectListing.action?districtCode="+node.data.id+"&includeChild="+chkIncludeChild+"&editable=0&_ts"+$time());
	});
	// 最后的10是上下边距和
...
<script type="text/javascript">
			function doChangeValue(value){
				if(value == 1){
					...
					contentLoad("${ctx}/bmp/caseCriticalviolation/caseCriticalviolation_selectListing.action?districtCode="+$('district_code').value+"&includeChild=0&_ts"+$time());
				} else {
					//包含
					$('chkIncludeChild').value = 1;
					contentLoad("${ctx}/bmp/caseCriticalviolation/caseCriticalviolation_selectListing.action?districtCode="+$('district_code').value+"&includeChild=1&_ts"+$time());
				}
			}
</script>
</html>