 ...
 * @author
 */
public class DiscloseSecrecyDistrictListTableCollector implements ParameterCollector<List<DistrictListTable>>{
	...
	public List<DistrictListTable> getData(Parameter param) {
		...
		List<DiscloseSecrecyStatDto> keySectionStatDtoList = discloseSecrecyService.queryDiscloseSecrecyByDistrict(new ArrayList<District>(district.getChildren()), true, organ,DiscloseSecrecy.class.getName());
		DistrictListTable districtTable = new DistrictListTable();
		districtTable.setTotalText("泄密案件");
