...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function doDownLoad(districtCode){
				window.location.href="${pageContext.request.contextPath}/bmp/part/stat_exportDistrict.action?district.districtCode="+districtCode;
			}

			function doDownLoad_1(districtCode){
				window.location.href="${pageContext.request.contextPath}/bmp/part/stat_exportLayer.action?district.districtCode="+districtCode;;
			}
		</script>
	...
	<body>
		...
		<div id="body_content" class="body_content">
			<div>
				<bmp:navigationBar topDistrict="${topDistrict}" district="${district}" url="${pageContext.request.contextPath}/bmp/part/zhtj_query_Detail.action"/>
			</div>
			<div class="panel">
				<div class="panel_header">
					<div class="panel_title panel_titleListIco">
						【${district.districtName }】<%=ConstantService.getConstant("District")%>要害部位统计
					</div>
					<!-- 右侧按钮 -->
					...
					<div class="eXtremeTable" >
						<table border="0"  cellspacing="0"  cellpadding="0"  class="tableRegion"  width="100%" >
							...
							<tbody class="tableBody">
								...
								<c:forEach items="${partStatDtoList}" var="o" varStatus="x">
									<tr height="36px;" class="${x.count % 2 == 0 ? 'odd' : 'even'}" onmouseover="this.className='highlight'" onmouseout="this.className='${x.count % 2 == 0 ? 'odd' : 'even' }'">
										<td><a href="${pageContext.request.contextPath}/bmp/part/zhtj_queryByDistrict.action?district.districtCode=${o.code}">${o.name}</a></td>
			...
			<c:if test="${not empty childrenStatDtoList}">
				...
				<div class="panel" style="margin-top: -15px;">
					<div class="panel_header">
						<div class="panel_title panel_titleListIco">
							<h2>
								【${district.districtName }】<%=ConstantService.getConstant("District_Area")%>要害部位统计
							</h2>
						</div>
						<!-- 右侧按钮 -->
						<div class="panel_btn_bar pop_button_bar" align="right">
									...
									<c:forEach items="${childrenStatDtoList}" var="o" varStatus="x">
										<tr height="36px;" class="${x.count % 2 == 0 ? 'odd' : 'even'}" onmouseover="this.className='highlight'"  onmouseout="this.className='${x.count % 2 == 0 ? 'odd' : 'even' }'">
											<td>
												...
												<c:if test="${o.code != null}"><a href="${pageContext.request.contextPath}/bmp/part/zhtj_query_Detail.action?district.districtCode=${o.code}">${o.name}</a></c:if>
											</td>
											<td>${o.layerSecrecyLevel1}</td><!-- 直辖单位 -->
											<td>${o.layerSecrecyLevel2}</td>
											<td>${o.layerSecrecyLevel3}</td>
