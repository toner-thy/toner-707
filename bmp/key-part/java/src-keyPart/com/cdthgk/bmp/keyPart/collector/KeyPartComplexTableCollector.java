...
/**
 * <p>
 * 通过行政区划  统计   行政区划和直辖单位的  要害部位个数   这里不需要合计行
 *  这里的查询出来的数据只有一排哦  亲
 *
 *  //查询出来的列
 ...
 * @author
 */
public class KeyPartComplexTableCollector implements ParameterCollector<ComplexTable>{
	...
	public ComplexTable getData(Parameter param) {
		...
		//通过行政区划  统计   行政区划和直辖单位的  要害部位个数   这里不需要合计
		List<ZongHeTongJiStatDto> list = partService.count_District(districtList, false);
		Iterator<ZongHeTongJiStatDto> it = list.iterator();

		while(it.hasNext()){
			ZongHeTongJiStatDto item = it.next();
			//表头
			complexTable.setTitle("要害部位");
