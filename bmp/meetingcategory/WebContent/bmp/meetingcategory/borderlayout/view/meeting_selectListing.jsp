	...
	<head>
		...
		<script type="text/javascript">
			...
			function doExport(action){
				window.location.href=action+"?meeting.meetingCategory.meetingCategoryId=${meeting.meetingCategory.meetingCategoryId}&baomiju=true&districtCode=${districtCode}&includeChild=${includeChild}";
			}
		</script>
	...
	<body>
			...
			<div class="left">
			  	<s:if test="#request.allMeetingList.size>0">
						<div class="pop_button_bar">
							<a class="pop_button" onclick="doExport('${ctx}/bmp/meeting/meeting_export.action');this.blur();" href="###">
								<span>导出</span>
							</a>
						  </div>
					</s:if>
		...
		<div class="body_content">
			...
			<cp:search show="true" divId="cp002">
				<s:form action="meeting_selectListing" id="meeting_release_myList_form" theme="simple">
					<!-- 隐藏域 -->
					...
					<div class="panel_title panel_titleListIco">
						...
						<!--判断是否查看下级1查看，0不查看-->
							<c:if test="${includeChild ne '1'}">
								${district.districtName}<dis:levelText district="${district}"></dis:levelText>级机关单位 - 【${meeting.meetingCategory.categoryName}】类型会议列表
							</c:if>
							<c:if test="${includeChild eq '1'}">
								${district.districtName} - 【${meeting.meetingCategory.categoryName}】类型会议列表
							</c:if>
					</div>
						<!-- 右侧按钮 -->
					...
					<div class="panel_content">
						<s:if test="#request.allMeetingList.size>0">
							<ec:table items="allMeetingList" var="aml" tableId="allMeetingList" border="0"
								action="${ctx}/bmp/meeting/meeting_selectListing.action"
								...
								<ec:row>
									...
									<ec:column property="meetingName" title="涉密会议（活动）名称" width="15%" style="line-height:20px;padding:5px;">
									...
									<ec:column property="meetingTime" title="开会日期" parse="yyyy-MM-dd HH:mm:ss" cell="date" width="12%" style="line-height:20px;padding:5px;"/>
									<ec:column property="null" title="涉密等级" width="15%" style="line-height:20px;padding:5px;">
									   <dictionary:text tableCode="bmp" fieldCode="secrecy_level_thing"
									optionValue="${aml.secrecyLevel}"/>
									</ec:column>

									<ec:column property="place"  title="地点" width="15%" style="line-height:20px;padding:5px;" cell="text" alias="size=10"/>
