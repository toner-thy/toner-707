 ...
 * @author
 */
public class SecrecyNetworkDistrictListTableCollector implements ParameterCollector<List<DistrictListTable>>{
	...
	public List<DistrictListTable> getData(Parameter param) {
		...
		List<ZongHeTongJiStatDto> list = secrecyNetworkService.count_SecrecyNetwork_District(new ArrayList<District>(district.getChildren()), true);

		//设置表头
		DistrictListTable districtTable = new DistrictListTable();
		districtTable.setTotalText("涉密网络");
