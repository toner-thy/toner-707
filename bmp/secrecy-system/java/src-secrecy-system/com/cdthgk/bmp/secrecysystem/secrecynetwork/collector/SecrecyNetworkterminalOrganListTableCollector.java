 ...
 * @author
 */
public class SecrecyNetworkterminalOrganListTableCollector implements ParameterCollector<List<OrganListTable>>{
	...
	public List<OrganListTable> getData(Parameter param) {
		...
		//查询  统计出吧单位下 涉密网络终端 的数量  按照密级统计
		List<ZongHeTongJiStatDto> statList = secrecyNetworkterminalService.count_SecrecyNetworkterminal_Organ(district,organ, false);

		// 设置头部  表格的第一排
		OrganListTable organTable = new OrganListTable();
		organTable.setOrganName("涉密网络终端");
