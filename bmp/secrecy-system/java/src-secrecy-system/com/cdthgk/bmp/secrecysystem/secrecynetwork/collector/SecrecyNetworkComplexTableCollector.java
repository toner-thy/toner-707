...
/**
 * <p>
 * 通过行政区划  统计   行政区划和直辖单位的  涉密网络个数   这里不需要合计行
 *  这里的查询出来的数据只有一排哦  亲
 *
 *  //查询出来的列
 ...
 * @author
 */
public class SecrecyNetworkComplexTableCollector implements ParameterCollector<ComplexTable>{

	/*** Spring依赖注入 服务层注入 ***/
	private SecrecyNetworkService secrecyNetworkService;

	/**
	 * 通过行政区划  统计   行政区划和直辖单位的涉密网络个数
	 * 不需要合计行
	 */
	@Override
	public ComplexTable getData(Parameter param) {
		...
		//通过行政区划  统计   行政区划和直辖单位的  涉密网络个数   这里不需要合计
		List<ZongHeTongJiStatDto> list = secrecyNetworkService.count_SecrecyNetwork_District(districtList, false);
		Iterator<ZongHeTongJiStatDto> it = list.iterator();

		while(it.hasNext()){
			ZongHeTongJiStatDto item = it.next();
			//表头
			complexTable.setTitle("涉密网络");
