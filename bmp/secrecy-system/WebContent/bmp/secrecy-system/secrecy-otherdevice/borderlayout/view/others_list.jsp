		...
		<div class="body_content" >
			...
			<cp:start defaultTitle="其他涉密设备简介" ctx="${ctx}" icoPath="/bmp/secrecy-system/secrecy-otherdevice/borderlayout/skin/blue/img/others_list_cpIco.gif">
				...
				<cp:search show="false" divId="cp002">
					<s:form namespace="/secrecysystem/secrecyOthers" action="others_list.action" method="post" id="queryForm" theme="simple">
					...
					<div class="panel_title panel_titleListIco">
						...
						<c:if test="${fromQuery eq '1'}">
							...
							<c:if test="${checkLower ne '1'}">
								${district.districtName}<dis:levelText district="${district}"></dis:levelText>级机关单位 - 其他涉密设备列表
							</c:if>
							<c:if test="${checkLower eq '1'}">
								${district.districtName} - 其他涉密设备列表
							</c:if>
						</c:if>
					</div>
				</div>
				<div class="panel_content panel_content_table">
					<s:if test="#attr.secrecyOthersList.size>0">
						<ec:table items="secrecyOthersList" var="secrecyOthers"
							...
							action="${ctx}/secrecysystem/secrecyothers/others_list.action"
