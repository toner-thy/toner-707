...
<html>
	<head>
		...
		<script type="text/javascript">
 			function doSecrecyMobilestorageTotalView( mediaType) {
 				var action = "${ctx}/secrecysystem/secrecymobilestoragemedia/mobilestoragemedia_countData.action";
 				TabUtil.openAsTab({
 					//url : action + "?secrecyMobilestoragemedia.mediaType="+ mediaType + "&countType=${countType}" +"&t_date=" + new Date().getTime(),
 					url : action + "?secrecyMobilestoragemedia.mediaType="+ mediaType + "&countType=organ" +"&t_date=" + new Date().getTime(),
 					title : '涉密移动存储介质合计-列表'
 				});
 			}

 			function doSecrecyMobilestorageCommonView(secrecyLevel, mediaType){
 				var action ="${ctx}/secrecysystem/secrecymobilestoragemedia/mobilestoragemedia_countSubData.action";
 				TabUtil.openAsTab({
 					...
 					url : action + '?secrecyMobilestoragemedia.secrecyLevel=' + secrecyLevel + "&countType=organ" + '&secrecyMobilestoragemedia.mediaType='+ mediaType +'&t_date='+ new Date().getTime(),
 					title : '涉密移动存储介质-列表'
 				});
 			}
					...
					<table style="width: 100%;">
						<tr>
							<td	class="td_img_left">
								<!-- 图标 -->
								<img src="${ctx}/bmp/images/index/mobileStoragemedia/ico.jpg">
							</td>
															...
															<c:forEach var="aKeyValue" items="${aResultMap.value }">
																<c:if test="${aKeyValue.key eq typeOption.optionValue }">
																	<c:choose>
																		...
																		<c:otherwise>
																			<a href="###" onclick="doSecrecyMobilestorageCommonView('${aResultMap.key }','${aKeyValue.key }')">
																				${aKeyValue.value}
																			</a>
																		</c:otherwise>
																	</c:choose>
									...
									<tr align="right">
										...
										<c:forEach var="typeOption" items="${typeList }" >
											<td class="td_total_right">
												...
												<c:otherwise>
													<a href="###" onclick="doSecrecyMobilestorageTotalView('${typeOption.optionValue}')">
														${totalNum }
													</a>
												</c:otherwise>
											</c:choose>
