...
<html>
	<head>
		...
		<script src="${ctx}/resources/js/miftree/1.2/mif.tree.js" type="text/javascript"></script>
	...
	<body>
		...
		<div id="body_content" class="body_content">
			...
			<div class="tree_right_content"><!-- 右边表的内容  src=初始化内容 -->
				<iframe name="contentIframe" id="contentIframe" scrolling="auto" src="${ctx}/secrecysystem/secrecymobilestoragemedia/mobilestoragemedia_query_list.action?fromQuery=1&checkLower=1" height="100%" width="100%" frameborder="0"></iframe>
			</div>
		</div>
	</body>
</html>
<script type="text/javascript">
	...
	tree.addEvent('select',function(node){
		...
		contentLoad("${ctx}/secrecysystem/secrecymobilestoragemedia/mobilestoragemedia_query_list.action?districtCode="+node.data.id+"&fromQuery=1&checkLower="+chkIncludeChild+"&editable=0&_ts"+$time());
	});
	// 最后的10是上下边距和
	...
	function doChangeValue(value){
		if(value == 1){
			...
			contentLoad("${ctx}/secrecysystem/secrecymobilestoragemedia/mobilestoragemedia_query_list.action?districtCode="+$('district_code').value+"&fromQuery=1&checkLower=0&_ts"+$time());
		} else {
			$('chkIncludeChild').value = 1;
			contentLoad("${ctx}/secrecysystem/secrecymobilestoragemedia/mobilestoragemedia_query_list.action?districtCode="+$('district_code').value+"&fromQuery=1&checkLower=1&_ts"+$time());
		}
	}
