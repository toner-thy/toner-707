		...
		<div class="body_content" >
			...
			<cp:start defaultTitle="涉密移动存储介质简介" ctx="${ctx}" icoPath="/bmp/secrecy-system/secrecy-otherdevice/borderlayout/skin/blue/img/secrecyMobilestoragemedia_list_cpIco.gif">
				...
				<cp:search show="false" divId="cp002">
					<s:form namespace="/secrecysystem/secrecymobilestoragemedia" action="mobilestoragemedia_list.action" method="post" id="queryForm" theme="simple">
						...
						<table class="st" width="100%">
							<tr>
								...
								<td class="tbValue fl">
									<dictionary:select tableCode="bmp" fieldCode="media_type" id="secrecyMobilestoragemedia.mediaType" name="secrecyMobilestoragemedia.mediaType" title="true" titleText="" optionValue="${secrecyMobilestoragemedia.mediaType }" style="width:130px;"></dictionary:select>
								</td>
								<td class="tbLable fr">涉密等级：</td>
								<td class="tbValue fl">
									<dictionary:select tableCode="bmp" fieldCode="secrecy_level_thing" id="secrecyMobilestoragemedia.secrecyLevel" name="secrecyMobilestoragemedia.secrecyLevel" title="true" titleText="" optionValue="${secrecyMobilestoragemedia.secrecyLevel }" style="width:130px;"></dictionary:select>
								</td>
							</tr>
							<tr>
								<td class="tbLable fr">责任人：</td>
								<td class="tbValue fl">
									<input type="text" id="secrecyMobilestoragemedia.dutyPerson.name" name="secrecyMobilestoragemedia.dutyPerson.name" value="${secrecyMobilestoragemedia.dutyPerson.name }" />
								</td>
								<td class="tbLable fr" nowrap="nowrap">属于要害部门部位：</td>
								<td class="tbValue fl">
									<dictionary:select tableCode="bmp" fieldCode="is_section_part" id="secrecyMobilestoragemedia.isbelongKeydepartment" name="secrecyMobilestoragemedia.isbelongKeydepartment" title="true" titleText="" optionValue="${secrecyMobilestoragemedia.isbelongKeydepartment }" style="width:130px;"></dictionary:select>
								</td>
							</tr>
							<tr>
					...
					<div class="panel_title panel_titleListIco">
						...
						<c:if test="${fromQuery eq '1'}">
							...
							<c:if test="${checkLower ne '1'}">
								${district.districtName}<dis:levelText district="${district}"></dis:levelText>级机关单位 - 涉密移动存储介质列表
							</c:if>
							<c:if test="${checkLower eq '1'}">
								${district.districtName} - 涉密移动存储介质列表
							</c:if>
						</c:if>
					</div>
				</div>
				<div class="panel_content panel_content_table">
					<s:if test="#attr.secrecyMobilestoragemediaList.size>0">
						<ec:table items="secrecyMobilestoragemediaList" var="secrecyMobilestoragemedia"
							...
							action="${ctx}/secrecysystem/secrecymobilestoragemedia/mobilestoragemedia_list.action"
							...
							<ec:row>
								...
								<ec:column property="dutyPerson.name" title="责任人" cell="text" alias="size=4" width="8%"/>
								<ec:column property="mediaNo" title="编号" cell="text" alias="size=7" width="10%"/>
								<ec:column property="mediaSeq" title="序列号" cell="text" alias="size=7" width="10%"/>
