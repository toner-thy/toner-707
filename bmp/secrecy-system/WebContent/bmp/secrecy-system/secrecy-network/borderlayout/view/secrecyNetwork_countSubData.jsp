		...
		<div class="body_content" >
			...
			<cp:start defaultTitle="涉密网络简介" ctx="${ctx}" icoPath="/bmp/secrecy-system/secrecy-network/borderlayout/skin/blue/img/list_cpIco.gif">
				...
				<cp:search show="false" divId="cp002">
					<form action="<s:url action="secrecyNetwork_countSubData.action" namespace="/bmp/secrecynetwork"/>" method="post" id="queryForm" theme="simple">
						...
						<input type="hidden" id="secrecyNetwork.secrecyLevel" name="secrecyNetwork.secrecyLevel" value="${secrecyNetwork.secrecyLevel}" />
				...
				<div class="panel_content panel_content_table">
					<s:if test="#request.secrecyNetworkList.size>0">
						<ec:table items="secrecyNetworkList" var="secrecyNetwork" tableId="secrecyNetworkList" border="0"
							action="${ctx}/bmp/secrecynetwork/secrecyNetwork_countSubData.action"
							...
							<ec:row>
								...
								<ec:column property="null" title="网络终端数量" width="10%">
								    <c:if test="${secrecyNetwork.networkNum eq null }">0</c:if>
									<c:if test="${secrecyNetwork.networkNum ne null }">${secrecyNetwork.networkNum}</c:if>
								</ec:column>
