...
<html>
	<head>
		...
		<script type="text/javascript">
 			function doSecrecyNetworkterminalTotalViewLayer( computerType) {
 				var action = "${ctx}/bmp/secrecynetworkterminal/secrecyNetworkterminal_countData.action";
 				TabUtil.openAsTab({
 					url : action + "?secrecyNetwork.secrecyComputer.computerType="+ computerType + "&countType=${countType}"+ '&secrecyNetworkterminal.secrecyComputer.computerType='+ computerType +"&t_date=" + new Date().getTime(),
 					title : '涉密网络终端合计-列表'
 				});
 			}

 			function doSecrecyNetworkterminalCommonViewLayer(secrecyLevel, computerType){
 				var action ="${ctx}/bmp/secrecynetworkterminal/secrecyNetworkterminal_countSubData.action";
 				TabUtil.openAsTab({
 					url : action + '?secrecyNetworkterminal.secrecyLevel=' + secrecyLevel + "&countType=${countType}" + '&secrecyNetworkterminal.secrecyComputer.computerType='+ computerType +'&t_date='+ new Date().getTime(),
 					title : '涉密网络终端设备-列表'
 				});
 			}
					...
					<table style="width: 100%;">
						<tr>
							<td	class="td_img_left">
								<!-- 图标 -->
								<img src="${ctx}/bmp/images/index/secrecyNetWork/ico.jpg">
							</td>
															...
															<c:forEach var="aKeyValue" items="${aResultMap.value }">
																<c:if test="${aKeyValue.key eq typeOption.optionValue }">
																	<c:choose>
																		...
																		<c:otherwise>
																			<a href="###" onclick="doSecrecyNetworkterminalCommonViewLayer('${aResultMap.key }','${aKeyValue.key }')">
																				${aKeyValue.value}
																			</a>
																		</c:otherwise>
																	</c:choose>
									...
									<tr align="right">
										...
										<c:forEach var="typeOption" items="${typeList }" >
											<td class="td_total_right">
												...
												<c:otherwise>
													<a href="###" onclick="doSecrecyNetworkterminalTotalViewLayer('${typeOption.optionValue}')">
														${totalNum }
													</a>
												</c:otherwise>
											</c:choose>
