...
<html>
	<head>
		...
		<script type="text/javascript">
 			function doSecrecyNetworkterminalTotalView( computerType) {
 				var action = "${ctx}/bmp/secrecynetworkterminal/secrecyNetworkterminal_countData.action";
 				TabUtil.openAsTab({
 					url : action + "?secrecyNetwork.secrecyComputer.computerType="+ computerType + "&countType=${countType}" +"&t_date=" + new Date().getTime(),
 					title : '涉密网络终端合计-列表'
 				});
 			}

 			function doSecrecyNetworkterminalCommonView(secrecyLevel, computerType){
															...
															<c:forEach var="aKeyValue" items="${aResultMap.value }">
																<c:if test="${aKeyValue.key eq typeOption.optionValue }">
																	<c:choose>
																		...
																		<c:otherwise>
																			<a href="###" onclick="doSecrecyNetworkterminalCommonView('${aResultMap.key }','${aKeyValue.key }')">
																				${aKeyValue.value}
																			</a>
																		</c:otherwise>
																	</c:choose>
									...
									<tr align="right">
										...
										<c:forEach var="typeOption" items="${typeList }" >
											<td class="td_total_right">
												...
												<c:otherwise>
													<a href="###" onclick="doSecrecyNetworkterminalTotalView('${typeOption.optionValue}')">
														${totalNum }
													</a>
												</c:otherwise>
											</c:choose>
