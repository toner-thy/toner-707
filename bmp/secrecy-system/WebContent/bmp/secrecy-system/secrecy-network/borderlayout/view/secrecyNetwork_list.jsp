		...
		<div class="body_content" >
			...
			<cp:start defaultTitle="涉密网络简介" ctx="${ctx}" icoPath="/bmp/secrecy-system/secrecy-network/borderlayout/skin/blue/img/list_cpIco.gif">
				...
				<cp:search show="false" divId="cp002">
					<form action="<s:url action="secrecyNetwork_list.action" namespace="/bmp/secrecynetwork"/>" method="post" id="queryForm" theme="simple">
					...
					<div class="panel_title panel_titleListIco">
						...
						<c:if test="${fromQuery eq '1'}">
							...
							<c:if test="${checkLower ne '1'}">
								${district.districtName}<dis:levelText district="${district}"></dis:levelText>级机关单位 - 涉密网络列表
							</c:if>
							<c:if test="${checkLower eq '1'}">
								${district.districtName} - 涉密网络列表
							</c:if>
						</c:if>
					</div>
				</div>
				<div class="panel_content panel_content_table">
					<s:if test="#request.secrecyNetworkList.size>0">
						<ec:table items="secrecyNetworkList" var="secrecyNetwork" tableId="secrecyNetworkList" border="0"
							action="${ctx}/bmp/secrecynetwork/secrecyNetwork_list.action"
