		...
		<div class="body_content" >
			...
			<cp:start defaultTitle="涉密网络简介" ctx="${ctx}" icoPath="/bmp/secrecy-system/secrecy-network/borderlayout/skin/blue/img/list_cpIco.gif">
				...
				<cp:search show="false" divId="cp002">
					<form action="<s:url action="secrecyNetworkterminal_countSubData.action" namespace="/bmp/secrecynetworkterminal"/>" method="post" id="queryForm" theme="simple">
						<input type="hidden" name="countType" id="countType" value="${countType }" />
						<input type="hidden" name="secrecyNetworkterminal.secrecyComputer.computerType" id="secrecyNetworkterminal.secrecyComputer.computerType" value="${secrecyNetworkterminal.secrecyComputer.computerType }" />
						<input type="hidden" id="secrecyNetworkterminal.secrecyLevel" name="secrecyNetworkterminal.secrecyLevel" value="${secrecyNetworkterminal.secrecyLevel}" />
				...
				<div class="panel_content panel_content_table">
					<s:if test="#request.secrecyNetworkterminalList.size>0">
						<ec:table items="secrecyNetworkterminalList" var="secrecyNetworkterminal" tableId="secrecyNetworkterminalTable" border="0"
							action="${ctx}/bmp/secrecynetworkterminal/secrecyNetworkterminal_countSubData.action"
							...
							<ec:row>
								...
								<ec:column property="secrecyNetwork.name" title="涉密网络名称" cell="text" alias="size=10" width="10%"/>
								<ec:column property="dutyPerson.name" title="责任人"></ec:column>
								...
								<ec:column property="null" title="机型">
								...
								<ec:column property="secrecyComputer.computerNo" title="设备编号"></ec:column>
								<ec:column property="secrecyComputer.diskSeq" title="硬盘序列号"></ec:column>
								<ec:column property="ipAddress" title="IP地址"></ec:column>
								<ec:column property="macAddress" title="MAC地址"></ec:column>
								<ec:column property="null" title="是否属于<br/>要害部门、部位">
								...
								<ec:column property="null" title="是否安装<br/>保密技术防护<br/>专用系统<br/>（三合一）">
