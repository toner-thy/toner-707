...
<html>
	<head>
		...
		<script type="text/javascript">
 			function doSecrecyNetworkTotalViewLayer( networkType) {
 				var action = "${ctx}/bmp/secrecynetwork/secrecyNetwork_countData.action";
 				TabUtil.openAsTab({
 					url : action + "?secrecyNetwork.networkType="+ networkType + "&countType=${countType}" +"&t_date=" + new Date().getTime(),
 					title : '涉密网络合计-列表'
 				});
 			}

 			function doSecrecyNetworkCommonViewLayer(secrecyLevel, networkType){
 				var action ="${ctx}/bmp/secrecynetwork/secrecyNetwork_countSubData.action";
 				TabUtil.openAsTab({
 					url : action + '?secrecyNetwork.secrecyLevel=' + secrecyLevel + "&countType=${countType}" + '&secrecyNetwork.networkType='+ networkType +'&t_date='+ new Date().getTime(),
 					title : '涉密网络设备-列表'
 				});
 			}
															...
															<c:forEach var="aKeyValue" items="${aResultMap.value }">
																<c:if test="${aKeyValue.key eq typeOption.optionValue }">
																	<c:choose>
																		...
																		<c:otherwise>
																			<a href="###" onclick="doSecrecyNetworkCommonViewLayer('${aResultMap.key }','${aKeyValue.key }')">
																				${aKeyValue.value}
																			</a>
																		</c:otherwise>
																	</c:choose>
									...
									<tr align="right">
										...
										<c:forEach var="typeOption" items="${typeList }" >
											<td class="td_total_right">
												...
												<c:otherwise>
													<a href="###" onclick="doSecrecyNetworkTotalViewLayer('${typeOption.optionValue}')">
														${totalNum }
													</a>
												</c:otherwise>
											</c:choose>
