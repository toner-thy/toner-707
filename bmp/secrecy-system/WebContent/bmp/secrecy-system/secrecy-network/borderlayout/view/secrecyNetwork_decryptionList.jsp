		...
		<div class="body_content" >
			...
			<cp:start defaultTitle="涉密网络简介" ctx="${ctx}" icoPath="/bmp/secrecy-system/secrecy-network/borderlayout/skin/blue/img/list_cpIco.gif">
				...
				<cp:search show="false" divId="cp002">
					<form action="<s:url action="secrecyNetwork_decryptionList.action" namespace="/bmp/secrecynetwork"/>" method="post" id="queryForm" theme="simple">
						...
						<table class="st" width="100%">
							<tr>
								...
								<td class="tbValue fl">
									<dictionary:select tableCode="bmp" fieldCode="secrecy_level_thing" id="secrecyNetworkClear.secrecyNetwork.secrecyLevel" name="secrecyNetworkClear.secrecyNetwork.secrecyLevel"
									 title="true" titleText="请选择" style="width: 130px;" optionValue="${secrecyNetworkClear.secrecyNetwork.secrecyLevel}"></dictionary:select>
								</td>
								<td class="tbLable fr">解除类型：</td>
								<td class="tbValue fl">
									<dictionary:select tableCode="bmp" fieldCode="clear_secrecy_type" id="secrecyNetworkClear.clearType" name="secrecyNetworkClear.clearType"
									 optionValue="${secrecyNetworkClear.clearType}" title="true" style="width:130px;"></dictionary:select>
								</td>
							</tr>
					...
					<div class="panel_title panel_titleListIco">
						...
						<c:if test="${fromQuery eq '1'}">
							...
							<c:if test="${checkLower ne '1'}">
								${district.districtName}<dis:levelText district="${district}"></dis:levelText>级机关单位 - 涉密网络密级解除列表
							</c:if>
							<c:if test="${checkLower eq '1'}">
								${district.districtName} - 涉密网络密级解除列表
							</c:if>
						</c:if>
					</div>
				</div>
				<div class="panel_content panel_content_table">
					<s:if test="#attr.secrecyNetworkClearList.size>0">
						<ec:table items="secrecyNetworkClearList" var="m" tableId="secrecyNetworkClearList" border="0"
							action="${ctx}/bmp/secrecynetwork/secrecyNetwork_decryptionList.action"
							...
							<ec:row>
								...
								<ec:column property="null" title="解除前密级" width="10%">
									<dictionary:text tableCode="bmp" fieldCode="secrecy_level_thing" optionValue="${m.secrecyNetwork.secrecyLevel}"/>
								</ec:column>
								...
								<ec:column property="null"  width="5%" title="详 情">
									<a href="###" onclick="doDetail('${m.secrecyNetwork.secrecyNetworkId}')">
