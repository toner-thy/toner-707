	...
	<head>
		...
		<script type="text/javascript">
					...
					if(!item.content.getContentWindow().needReload2){
						if (window.confirm("您确定放弃正在新增的内容吗？")) {
							if(item.content.getContentWindow().needReload){
								document.getElementById('rewardAndPenalty_list_form').submit();
							}
						} else {
							return false;
						}
			...
			function doEdit(action){
				...
				TabUtil.openAsTab({
					url : action+"?list_crd=${param.list_crd}&list_p=${param.list_p}&rewardAndPenalty.id="+items[0].value,
					title : '实施奖惩情况-编辑',
					onClose : function(tab, item) {
					if(!item.content.getContentWindow().needReload2){
						if (window.confirm("您确定放弃正在新增的内容吗？")) {
			...
			function doPrint(action){
		    	var item = EcTable.getCheckedItems();
		     	if(item!=null && item!=""){
			    	window.showModalDialog( action+'?rewardAndPenalty.id='+item[0].value+'&t_time='+new Date().getTime(),window,"dialogWidth=630px;dialogHeight=400px;status=no;directories=no;menubar=no;resizable=yes;crollbars=auto;help=no");
		     	}else{
       		...
       		//导出
       	  	function doExport(action){
       	     	$('rewardAndPenalty_list_form').submit();
		     	$('rewardAndPenalty_list_form').action="<s:url action="rewardAndPenalty_list.action" includeParams="true"/>";
       	  	}
		</script>
		...
		<div id="body_content" class="body_content" style="width: 99%;">
		   ...
		   <!-- 复合面板开始 -->
			<cp:start defaultTitle="设备分类简介" ctx="${ctx}" icoPath="/bmp/checkevent/borderlayout/skin/blue/img/list_.gif">
				<div id="cp001Btn" class="cpBtn" href="###" onmouseover="javascript:showCp(2,'cp001','实施奖惩情况简介');">关 于</div>
				<div id="cp002Btn" class="cpBtn_ov" href="###" onmouseover="javascript:showCp(2,'cp002','实施奖惩情况搜索');">查 询</div>
			</cp:start>
				<cp:msg show="false" divId="cp001">
					...
					<div class="cpMsgContext">
						实施奖惩情况可以帮助保密行政管理部门对事项（人员）名称的统一管理。
					</div>
				...
				<cp:search show="true" divId="cp002">
					<form action="rewardAndPenalty_list.action" id="rewardAndPenalty_list_form" name='queryform'>
						<table class="st" width="100%">
							...
							<tr>
								...
								<td class="tbValue fl">
									<input name="rewardAndPenalty.startTime" id="rewardAndPenalty.startTime" type="text" value="${rewardAndPenalty.startTime}" class="Wdate default" onFocus="WdatePicker({maxDate:'#F{$dp.$D(\'rewardAndPenalty.endTime\')||\'2020-10-01 00:00:00\'}'})" readonly="readonly">-
									<input name="rewardAndPenalty.endTime" id="rewardAndPenalty.endTime" type="text" value="${rewardAndPenalty.endTime}" class="Wdate" onFocus="WdatePicker({minDate:'#F{$dp.$D(\'rewardAndPenalty.startTime\')}',maxDate:'2020-10-01 00:00:00'})" readonly="readonly">
								</td>
							</tr>
							<tr>
								<td class="tbLable fr">
								...
								<td class="tbValue fl">
									<input name="rewardAndPenalty.name" style="width: 43%" id="rewardAndPenalty.name" type="text" value="${rewardAndPenalty.name}">
								</td>
							</tr>
							<tr>
			...
			<div class="panel tMargin">
				...
				<s:if test="#request.list.size>0">
					<ec:table items="list" var="rewardAndPenalty" tableId="list" border="0"
						action="${ctx}/bmp/rewardAndPenalty/rewardAndPenalty_list.action"
