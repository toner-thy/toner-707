	...
	<body>
		<div id="body_content">
			...
			<div class="tree_right_content" style="border: 1px solid #B5CBD9;">
				<iframe name="technicTrainIframe" id="technicTrainIframe" scrolling="auto" src="${ctx}/bmp/technicTrain/technicTrain_allList.action" height="100%" width="100%" frameborder="0"></iframe>
			</div>
			<!-- 右侧内容结束 -->
		</div>
	</body>
...
<script type="text/javascript">
	function contentLoad(url) {
		document.getElementById("technicTrainIframe").src = url;
	}
	...
	tree.addEvent('select',function(node){
		contentLoad("${ctx}/bmp/technicTrain/technicTrain_allList.action?district.districtCode="+node.data.id+"&district.districtName="+node.name+"&showPagination=false");
	});
