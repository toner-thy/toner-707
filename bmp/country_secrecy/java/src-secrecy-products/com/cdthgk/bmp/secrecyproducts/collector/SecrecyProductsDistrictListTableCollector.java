 ...
 * @author
 */
public class SecrecyProductsDistrictListTableCollector implements ParameterCollector<List<DistrictListTable>>{
	...
	public List<DistrictListTable> getData(Parameter param) {
		...
		List<ZongHeTongJiStatDto> list = secrecyProductsService.count_SecrecyProducts_District(new ArrayList<District>(district.getChildren()), true);

		//设置表头
		DistrictListTable districtTable = new DistrictListTable();
		districtTable.setTotalText("密品");
