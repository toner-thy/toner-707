 ...
 * @author
 */
public class SecrecyProductsFashionTableCollector implements ParameterCollector<FashionTable>{

	/***** Spring依赖注入,  服务层注入  ***/
	private SecrecyProductsService secrecyProductsService; //密品的service

	/**
	 * //通过（单位）对象   查询该单位的   密品的统计情况    这里查询只返回的一条记录而已
	 */
	@Override
	public FashionTable getData(Parameter param) {
		...
		//通过（单位）对象   查询该单位的   密品的密级情况    这里查询只返回的一条记录而已
		List<ZongHeTongJiStatDto> objectList = secrecyProductsService.count_SecrecyProducts_Organ(null,organ, false);
		...
		if(objectList!=null) {
			...
			while(it.hasNext()){
				...
				fashionTable.setTitle("密品统计");
				fashionTable.setImgUrl("/bmp/statfk/borderlayout/resource/img/secrecyProducts.gif");
				fashionTable.setListAction("bmp/secrecyProducts/zhtj_OrganDetailList.action");
