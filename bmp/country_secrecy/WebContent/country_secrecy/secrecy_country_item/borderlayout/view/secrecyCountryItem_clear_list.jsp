...
<html>
	...
	<body>	<!-- 国家秘密事项解除信息列表 -->
		<div id="body_content" class="body_content">
		    <cp:start defaultTitle="国家秘密事项解除信息简介" ctx="${ctx}" icoPath="/country_secrecy/secrecy_country_item/borderlayout/skin/blue/img/list_cpIco.gif">
				<div id="cp001Btn" class="cpBtn_ov" href="###" onmouseover="javascript:showCp(2,'cp001','国家秘密事项解除简介');">关 于</div>
				<div id="cp002Btn" class="cpBtn" href="###" onmouseover="javascript:showCp(2,'cp002','国家秘密事项解除记录查询');">查 询</div>
			</cp:start>
					...
					<div class="panel_title panel_titleListIco">
						...
						<c:if test="${ywFlag eq '1'}">
							...
							<c:if test="${isChildren ne '1'}">
								${district.districtName}<dis:levelText district="${district}"></dis:levelText>级机关单位 - 国家秘密事项密级解除列表
							</c:if>
							<c:if test="${isChildren eq '1'}">
								${district.districtName} - 国家秘密事项密级解除列表
							</c:if>
						</c:if>
					</div>
				</div>
