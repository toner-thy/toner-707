...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			//导出
			function doExport_secrecycountryitemadd(){
				var frm = $('excel_secrecycountryitemadd_form');
				frm.action = "${ctx}/bmp/secrecycountryitem/export_Organ_CountrySecrecy_Add.action";
				frm.submit();
			}

			//回调方法
			function callBackFunction_add(beginDate, endDate, dateType){
				var url = "${ctx}/bmp/secrecycountryitem/count_Organ_CountrySecrecy_Add.action?&organId=${param.organId}&dateType="+dateType;
				if(beginDate!="") {
					url += "&beginDate="+beginDate+"&endDate="+endDate;
				}
				//console.log(url);
				$('count_Organ_CountrySecrecy_Add').getParent().load(url);
			}
	...
	<body>
		...
		<div style="padding:5px;" id="count_Organ_CountrySecrecy_Add">
			<!-- 年度过滤器 -->
			<yearStat:yearstat_query actionUrl="${ctx}/bmp/secrecycountryitem/count_Organ_CountrySecrecy_Add.action"
			callBackFunction="callBackFunction_add" yearStatProvider="${yearStatProvider }"></yearStat:yearstat_query>
			...
			<div class="panel tMargin" style="overflow-y: auto;">
				<div class="panel_header">
					...
					<div class="panel_btn_bar pop_button_bar">
						<a href="###" id="doExport"  onclick="doExport_secrecycountryitemadd()" class="pop_button"><span>导出</span></a>
					</div>
				</div>
			...
			<form action="" name="excel_secrecycountryitemadd_form" id="excel_secrecycountryitemadd_form" enctype="application/x-www-form-urlencoded">
