...
<script type="text/javascript">
	...
	function doEdit(action) {
		...
		window.location.href = action+"?sysQuestion.questionId=" + items[0].value + "&_ts="+ new Date().getTime();
	}
	...
	function doDetail(id) {
		if( id!=null && id!="" ){
				$ENV.dialog.open({
					url : "${ctx}/bmp/sysQuestion/sysQuestion_detail.action?sysQuestion.questionId=" + id + "&_ts=" + new Date().getTime(),
					width : 0.8,
					height : 0.9,
					title : '详情'
				});
	...
	<div class="body_content">
						...
						<tr>
							...
							<td width="*" valign="top">
								<div id="cp001"
									...
									<!-- 模块简介 -->
									<div class="cpMsgTitle">关于常见问题问答</div>
								...
								<div id="cp002"
									...
									<form method="post" id="queryform" action="${ctx }/bmp/sysQuestion/sysQuestion_list.action">
										<table class="st" width="100%">
											<tr>
												<td class="tbLable fr">问题标题：</td>
												<td class="tbValue fl"><input name="sysQuestion.questionTitle" value="${sysQuestion.questionTitle }" type="text"/></td>
												<td class="tbLable fr">问题内容：</td>
												<td class="tbValue fl">
													<input type="text" name="sysQuestion.questionContent" value="${sysQuestion.questionContent }" />
												</td>
											</tr>
											<tr>
		...
		<div class="panel tMargin">
			<div class="panel_header">
				<div class="panel_title panel_titleListIco">常见问题问答列表</div>
				<div class="panel_btn_bar pop_button_bar"></div>
			</div>
			<div class="panel_content panel_content_table">
				<s:if test="#request.questionList.size>0">
				<s:property value="request.questionList.size"/>
					<ec:table items="questionList" var="dataRow" tableId="questionList" border="0"
						action="${actionRequestURI}"
						imagePath="${ctx}/platform/template/borderlayout/skin/blue/img/ec/*.gif"
						width="100%" retrieveRowsCallback="limit" sortRowsCallback="limit"
						filterable="false" autoIncludeParameters="true" sortable="false">
						<ec:row>
							<ec:column property="questionId" alias="questionId_checkbox" cell="checkbox" headerCell="checkbox" width="5%"/>
							<ec:column property="questionTitle" title="问题标题" width="15%" cell="text" alias="size=18"/>
							<ec:column property="questionContent" title="问题内容" width="" cell="text" alias="size=28"/>
							<ec:column property="createTime" title="创建时间" width="10%" cell="date" format="yyyy-MM-dd" />
							<ec:column property="createperson.userInfo.name" title="创建人" width="10%" cell="text" alias="size=18"/>
							<ec:column property="null" title="详 情" width="10%">
								<a href="###" onclick="doDetail('${dataRow.questionId}');"><img src="${ctx}/platform/template/borderlayout/skin/blue/img/btn/display.gif" border="0" title="显示详情"/></a>
							</ec:column>
						</ec:row>
					</ec:table>
				</s:if>
